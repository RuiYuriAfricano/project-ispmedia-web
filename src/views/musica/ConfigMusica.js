...
const ConfigMusica = ({ idEditMusica, onClose }) => {
  ...
  const handleAddMusica = async () => {
    ...
    if (capaMusica) {
      newCapaMusicaName = `${idEditMusica || (response1.data.length === 0 ? 0 : (response1.data[response1.data.length - 1].codMusica) + 1)}-${tituloMusica}-capa${capaMusica.name.slice(capaMusica.name.lastIndexOf('.'))}`;
    ...
    if (ficheiroMusical) {
      newFicheiroMusicalName = `${idEditMusica || (response1.data.length === 0 ? 0 : (response1.data[response1.data.length - 1].codMusica) + 1)}-${tituloMusica}${ficheiroMusical.name.slice(ficheiroMusical.name.lastIndexOf('.'))}`;
